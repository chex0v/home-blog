...
@section('content')
    <div class="login">
        <form class="login__form" action="{{ route('admin.login.authenticate') }}" method="post">
            @csrf
            ...
            <label class="login__label">
                ...
                <input class="login__input_text" type="password" name="password" placeholder="Введите пароль" />
                @error('password')
                    <span class="login__error">{{ $message }}</span>
                @enderror
            </label>
